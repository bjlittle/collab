...
def callback() -> None:
    ...
    tmesh = mesh.threshold(240, scalars=scalars, method="lower")
    tmesh.active_scalars_name = scalars
    plotter.add_mesh(
        tmesh, cmap=cmap, clim=clim, show_scalar_bar=False, lighting=False, name=scalars
    )
    actor.SetText(3, unit.num2date(time[t]).strftime(fmt))


# sort the assets in date ascending date order
path = Path("./assets/tropical")
