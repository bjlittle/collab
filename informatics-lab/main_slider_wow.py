...
class Frame:
    ...
    cc: np.ndarray       # mesh cell centers for each site
...
def prepare(fname: PathLike, mesh: pv.PolyData) -> Frame:
    ...
    cc = mesh.cell_centers().points[cids]

    frame = Frame(nobs=nobs, index=index, cids=cids, sites=sites, pois=pois, singles=singles, neighbours=neighbours, dt=dt, cc=cc, data=data, clim=clim)
...
def callback_slider(step) -> None:
    ...
    actor_dt.SetText(3, frame.dt[step])
    trend = ""
    ...
    actor_av.SetText(0, f"Mean={step_average:.2f}K ({trend})")

    labels = [f"{ob:.1f}K" for ob in obs[frame.singles]]
    kwargs = dict(
        always_visible=True,
        shape="rounded_rect",
        shape_opacity=1,
    ...
    plotter.add_point_labels(frame.cc[frame.singles], labels, name="cc_single", **kwargs)

    if frame.neighbours:
        mask = [cids[0] for cids in frame.neighbours.values()]
        points = frame.cc[mask]
        labels = [f"{ob:.1f}K" for ob in obs[mask]]
        plotter.add_point_labels(points, labels, name="cc_shared", **kwargs)


def callback_checkbox(flag):
    global actor_lbs
    global actor_pts

    if flag:
        actor_lbs.SetVisibility(True)
        if actor_pts is not None:
            actor_pts.SetVisibility(False)
    else:
        actor_lbs.SetVisibility(False)
        if actor_pts is None:
            actor_pts = plotter.add_points(
                frame.pois, color="red", point_size=5, render_points_as_spheres=True,
            )
...
actor_pts = None
actor_lbs = plotter.add_point_labels(
    frame.pois, frame.sites, point_color="red", point_size=5, render_points_as_spheres=True,
    always_visible=True, shape="rounded_rect", shape_opacity=0.5, font_size=8, text_color="white"
)

actor_dt = plotter.add_text(frame.dt[start], position="upper_right", font_size=10, shadow=True)
actor_av = plotter.add_text("", position="lower_left", font_size=10, shadow=True)
