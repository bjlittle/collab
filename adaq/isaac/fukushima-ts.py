...
def callback() -> None:
    ...
    idx = np.where((tmp['idx'] == 20100) | (tmp['idx'] == 20099) | (tmp['idx'] == 19900) | (tmp['idx'] == 19899))[0]
    if idx.size:
        line = ax.lines[0]
        if tstep == 1:
        ...
        xs = np.concatenate([xs, [dt]])
        value = tmp["data"][idx].mean()
        ys = np.concatenate([ys, [value]])
        line.set_xdata(xs)
        line.set_ydata(ys)
...
def callback_chart(flag: bool) -> None:
    global chart
    global p

    (p.add_chart if flag else p.remove_chart)(chart)
...
ax.set_ylabel(f"{str(cube_plume.units)}", fontsize=fontsize)
start = t.units.num2date(t.points[tstep])
end = t.units.num2date(t.points[n_tsteps])
...
idx = np.where((ptmp['idx'] == 20100) | (ptmp['idx'] == 20099) | (ptmp['idx'] == 19900) | (ptmp['idx'] == 19899))[0]
if idx.size:
    cells = ptmp.extract_cells(idx)
    p.add_mesh(cells, style="wireframe", color="lightgray")
    value = ptmp["data"][idx].mean()
    ax.plot([t.units.num2date(t.points[tstep])], value, color="red")

chart = pv.ChartMPL(fig, size=(0.3, 0.35), loc=(0.01, 0.6))
chart.background_color = (255, 255, 255, 0.5)
# p.add_chart(chart)
