...
def callback() -> None:
    ...
    p.add_mesh(tmp, name="plume", cmap=cmap, clim=clim, render=False, reset_camera=False, scalar_bar_args=sargs, show_edges=True)
...
# tmp.save(f"vtk/mesh_forecast_{forecasts[pstep]}_{tstep}.vtk")
tmp = pv.read(f"vtk/mesh_forecast_{forecasts[pstep]}_{tstep}.vtk").threshold(0.2)  #.extract_geometry().smooth_taubin(n_iter=50, pass_band=0.05)
p.add_mesh(tmp, scalars="data", name="plume", cmap=cmap, clim=clim, scalar_bar_args=sargs, show_edges=False)
p.add_coastlines()
...
p.add_callback(callback, interval=500)
